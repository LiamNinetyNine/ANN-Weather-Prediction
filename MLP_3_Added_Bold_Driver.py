...
class Node():
    ...
    def update_weights(self, learning_rate, alpha = 0.9):
        ...
        for weight, last_input in zip(self.weights, self.last_inputs):
            ...
            new_weight_without_momentum = new_weight
            weight_change = new_weight - weight
            momentum = alpha * weight_change
            new_weight += momentum
            self.updated_weights.append(new_weight)
            index += 1
            print(f"Updating the weights for node {self.index_in_layer} to {new_weight} from previous weight {weight} using momentum, the weight without momentum would be {new_weight_without_momentum} and the last input u_i has been recorded as {last_input}")
    ...
    def train(self, X_train, y_train):
        ...
        for _ in range(self.epochs):
            ...
            for row, y in zip(X_train,y_train):
                ...
                debug += 1
            new_MSE = float(sum(squared_error)/self.epochs)
            if new_MSE >= previous_MSE * 1.04: #if the error function increased by 4% or more then...
                # undo the weight change .....
                # decrease the error......
                pass
            mean_squared_error_per_epoch.append(new_MSE) # take the mean squared error of each prediction made on the dataset for each epoch
        end = perf_counter()
